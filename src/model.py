...
# version anterior de la funcion, la modifique por la de abajo para que solo sea consulta a feature store
def latitude_and_longitude_anterior(X: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    raw_data_rides = raw_data_rides[[
        'id_estacion_origen', 'lat_estacion_origen', 'long_estacion_origen']]
    raw_data_rides['id_estacion_origen'] = raw_data_rides['id_estacion_origen'].str.replace(
...
def get_pipeline(**hyperparams) -> Pipeline:
    ...
    return make_pipeline(
        ...
        MultiOutputRegressor(lgb.LGBMRegressor(
            **hyperparams, force_col_wise=True))
    )
