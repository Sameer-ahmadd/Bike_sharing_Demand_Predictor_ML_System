...
def download_one_file_of_raw_data(year: int) -> Path:  # , month: int) -> Path:
    ...
    try:
        ...
        if destination_path.is_file():
            ...
            raise Exception(
                f'Error al descargar {URL}: El archivo no se descargó correctamente.')
...
def unzip_and_convert_csv_to_parquet(year: int) -> Path:
    ...
    with zipfile.ZipFile(nombre_archivo_zip, 'r') as archivo_zip:
        ...
        # Suponiendo que el archivo CSV es el primer archivo en el zip
        nombre_archivo_csv = archivo_zip.namelist()[0]
        archivo_zip.extractall(RAW_DATA_DIR)  # (f"../data/raw/")

        # Leer el archivo CSV con pandas
        df = pd.read_csv(RAW_DATA_DIR / nombre_archivo_csv,
                         delimiter=',', decimal=".")  # RAW_DATA_DIR /
...
def load_raw_data(
    ...
    rides_one_year['pickup_location_id'] = rides_one_year['pickup_location_id'].str.replace(
        'BAEcobici', '').astype(int)
    # transform "pickup_datetime" to datetime
    rides_one_year['pickup_datetime'] = pd.to_datetime(
        rides_one_year['pickup_datetime'], format='%Y-%m-%d %H:%M:%S')
...
def transform_raw_data_into_ts_data(
    ...
    agg_rides = rides.groupby(
        ['pickup_hour', 'pickup_location_id']).size().reset_index()
    ...
    agg_rides_all_slots = agg_rides_all_slots[agg_rides_all_slots['pickup_location_id'].isin(
        estaciones_2022)]
...
def transform_ts_data_into_features_and_target(
    ...
    for location_id in tqdm(location_ids):
        ...
        # Agregué el (output_seq_len) porque quiero esa cantidad de horas
        ...
        targets_one_location = pd.DataFrame(
            y, columns=[f'rides_next_{i+1}_hour' for i in range(output_seq_len)])
...
def get_cutoff_indices_features_and_target(
    ...
    # le agrego "output_seq_len" para introducirlo como variable
    subseq_last_idx = input_seq_len + output_seq_len
    indices = []
...
def transform_ts_data_into_dataset_comparable_with_predictions(
    ...
    for location_id in tqdm(location_ids):
        ...
        # numpy -> pandas
        targets_one_location = pd.DataFrame(
            y, columns=[f'real_rides_next_{i+1}_hour' for i in range(output_seq_len)])
